...
import com.suyh0201.business.config.CacheManagerConfiguration;
...
public class CacheMenuComponent {
    ...
    // 用来测试不同的缓存名称之间是独立的
    @Cacheable(value = CACHE_NAME + "2", key = "#id", unless = "#result == null", condition = "#id != null", cacheManager = CacheManagerConfiguration.MENU_CACHE_MANAGER)
    public CacheMenuEntity selectById2(Long id) {
        return selectById(id);
    }

    @CacheEvict(value = CACHE_NAME, key = "#entity.menuId", condition = "#entity != null && #entity.menuId != null", cacheManager = CacheManagerConfiguration.MENU_CACHE_MANAGER)
    ...
    @CacheEvict(value = CACHE_NAME, key = "#result", cacheManager = CacheManagerConfiguration.MENU_CACHE_MANAGER)
